...
function localInterpret(text) {
  ...
  // PREFIX / SYMBOL requests
  // Examples:
  // "add £ in front of all channel names"
  // "put 〢 before every channel"
  // "make all channels start with £"
  if (/(?:add|put|place|prefix|start|begin|prepend|beginning|front)/i.test(s)
      && /\b(?:all|every|each)\s+channels?\b/i.test(s)) {
    const symbolMatch = s.match(/(?:symbol|prefix|with|put|add|place|start|begin|prepend|beginning|front)(?:\s+the)?\s*[:=]?\s*["'()\[\]{}<>]*([^\s]+)["'()\[\]{}<>]*/i);
    let prefix = symbolMatch ? symbolMatch[1] : null;

    // Most reliable form: "symbol (X)" / "symbol X"
    const explicit = s.match(/\bsymbol\s*[:=]?\s*[\(\[\{<]?\s*([^\)\]\}>\s]+)\s*[\)\]\}>]?/i);
    if (explicit) prefix = explicit[1];

    // Parenthesized single symbol, e.g. "add this symbol (£)"
    const paren = s.match(/[\(\[]\s*([^\)\]]+)\s*[\)\]]/);
    if (paren) prefix = paren[1].trim();

    // "put £ in front..." / "add £ before..."
    if (!prefix) {
      const direct = s.match(/(?:add|put|place|prefix|prepend|start|begin)\s+([^\s]+)\s+(?:in\s+)?(?:the\s+)?(?:front|beginning|before|at\s+the\s+beginning)/i);
      if (direct) prefix = direct[1];
    }

    if (prefix) {
      prefix = prefix.trim();
      const hasExplicitSpace = /\b(?:with|using)\s+(?:a\s+)?space\b|\bspace\s+(?:after|between)\b/i.test(s);
      return {
        action: "rename",
        target: "channel",
        scope: "all",
...
"add £ in front of all channel names" -> {"action":"rename","target":"channel","scope":"all","mode":"prefix","prefix":"£","space":false}
"put 〢 before every channel with a space" -> {"action":"rename","target":"channel","scope":"all","mode":"prefix","prefix":"〢","space":true}
